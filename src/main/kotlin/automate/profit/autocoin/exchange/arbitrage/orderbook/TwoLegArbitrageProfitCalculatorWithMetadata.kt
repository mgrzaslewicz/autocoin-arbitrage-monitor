...
interface TransactionFeeAmountFunction {
    fun apply(exchange: Exchange, currencyPair: CurrencyPair, amount: BigDecimal): TransactionFeeAmount
}

interface WithdrawalFeeAmountFunction {
    fun apply(exchange: Exchange, currency: String, amount: BigDecimal): BigDecimal?
}

class CurrencyWithdrawalAndDepositStatusChecker(private val metadataService: AuthorizedMetadataServiceGateway) {
    /**
...
class TwoLegArbitrageProfitCalculatorWithMetadata(
    ...
    class DefaultBuilder(
        private val metadataService: AuthorizedMetadataServiceGateway,
        private val transactionFeeRatioWhenNotAvailableInMetadata: BigDecimal,
    ) {
        val transactionFeeAmountFunction = object : TransactionFeeAmountFunction {
            override fun apply(
                ...
                val takerFeeRange = metadataService
                    .getMetadata(exchange).currencyPairMetadata[currencyPair]
        ...
        val withdrawalFeeAmountFunction = object : WithdrawalFeeAmountFunction {
            override fun apply(exchange: Exchange, currency: String, amount: BigDecimal): BigDecimal? {
                val currencyMetadata =
                    metadataService.getMetadata(exchange = exchange).currencyMetadata[currency]
                return when {
    ...
    override fun getProfitBuyAtSecondExchangeSellAtFirst(
        ...
        val relativeProfitWithoutFees = when {
            ...
            else -> firstOrderBookBuyPrice.averagePrice.divide(
                secondOrderBookSellPrice.averagePrice,
                RoundingMode.HALF_EVEN
            ) - BigDecimal.ONE
        }
        val baseCurrencyAmountBeforeTransfer =
            firstOrderBookBuyPrice.baseCurrencyAmount.min(secondOrderBookSellPrice.baseCurrencyAmount)
        ...
        val baseCurrencyAmountMinusTransactionFeeAtSecondExchange =
            baseCurrencyAmountBeforeTransfer - (transactionFeeAmountBeforeTransfer.feeAmount)
        ...
        val baseCurrencyAmountMinusSecondExchangeWithdrawalFee =
            baseCurrencyAmountMinusTransactionFeeAtSecondExchange - (transferFeeAmount ?: BigDecimal.ZERO)
        ...
        val baseCurrencyAmountMinusTransactionFeeAtFirstExchange =
            baseCurrencyAmountMinusSecondExchangeWithdrawalFee - (transactionFeeAmountAfterTransfer.feeAmount)
        val amountWithAllFeesAppliedPlusProfit =
            baseCurrencyAmountMinusTransactionFeeAtFirstExchange.plus(
                baseCurrencyAmountMinusTransactionFeeAtFirstExchange.multiply(relativeProfitWithoutFees)
            )
    ...
    override fun getProfitBuyAtFirstExchangeSellAtSecond(
        ...
        val relativeProfitWithoutFees = when {
            currencyWithdrawalAndDepositStatusChecker.areWithdrawalsAndDepositsDisabled(currencyPairWithExchangePair) -> noProfit
            else -> secondOrderBookBuyPrice.averagePrice.divide(
                firstOrderBookSellPrice.averagePrice,
                RoundingMode.HALF_EVEN
            ) - BigDecimal.ONE
        }
        val baseCurrencyAmountBeforeTransfer =
            secondOrderBookBuyPrice.baseCurrencyAmount.min(firstOrderBookSellPrice.baseCurrencyAmount)
        ...
        val baseCurrencyAmountMinusTransactionFeeAtFirstExchange =
            firstOrderBookSellPrice.baseCurrencyAmount - (transactionFeeAmountBeforeTransfer.feeAmount)
        ...
        val baseCurrencyAmountMinusFirstExchangeWithdrawalFee =
            baseCurrencyAmountMinusTransactionFeeAtFirstExchange - (transferFeeAmount ?: BigDecimal.ZERO)
        ...
        val baseCurrencyAmountMinusTransactionFeeAtSecondExchange =
            baseCurrencyAmountMinusFirstExchangeWithdrawalFee - (transactionFeeAmountAfterTransfer.feeAmount)
        val amountWithAllFeesAppliedPlusProfit =
            baseCurrencyAmountMinusTransactionFeeAtSecondExchange.plus(
                baseCurrencyAmountMinusTransactionFeeAtSecondExchange.multiply(relativeProfitWithoutFees)
            )
        val relativeProfit = amountWithAllFeesAppliedPlusProfit
            .divide(baseCurrencyAmountBeforeTransfer, RoundingMode.HALF_EVEN)
            .minus(BigDecimal.ONE)
